...
SELECT p.titre, COUNT(t.idTache) AS nbTaches
FROM projet p
LEFT JOIN tache t ON t.idProjet = p.idProjet
GROUP BY p.idProjet;


--Durée de chaque projet en jours--
SELECT titre, DATEDIFF(dateFin, dateDeb) AS duree_jours
FROM projet;


--Tâches en retard--
SELECT t.titre AS Tache, p.titre AS Projet, t.etat, p.dateFin
FROM tache t
JOIN projet p ON t.idProjet = p.idProjet
WHERE t.etat <> 'terminee' AND p.dateFin < CURDATE();

--Nombre de projets par employé (responsable)--
SELECT e.nom, e.prenom, COUNT(p.idProjet) AS nbProjets
FROM employe e
LEFT JOIN projet p ON p.idChef = e.idEmp
GROUP BY e.idEmp;

--Employé qui a réalisé le plus grand nombre de tâches--
SELECT e.nom, e.prenom, COUNT(t.idTache) AS nbTaches
FROM employe e
JOIN projet p ON p.idChef = e.idEmp
JOIN tache t ON t.idProjet = p.idProjet
WHERE t.etat = 'terminee'
GROUP BY e.idEmp
ORDER BY nbTaches DESC
LIMIT 1;

--Dernière tâche ajoutée pour chaque projet--
SELECT p.titre AS Projet, t.titre AS DerniereTache, t.idTache
FROM projet p
JOIN tache t ON t.idProjet = p.idProjet
WHERE t.idTache = (
    SELECT MAX(idTache)
    FROM tache
    WHERE idProjet = p.idProjet
);
